// Estudio de IA — generación de imágenes con FLUX vía Replicate.
// Wilson elige preset, escribe prompt, genera 1-4 variantes, las descarga o las
// asigna como image_url de un producto.
...
function EstudioAI() {
  ...
  const [latest, setLatest] = useState(null) // { images: [], cost }
  const [history, setHistory] = useState([])
  const [products, setProducts] = useState([])
  const [assignTarget, setAssignTarget] = useState(null) // { imageUrl }
  ...
  const removeBg = async (imageUrl) => {
    setBusy(true)
    try {
      const r = await api.post('/api/ai/remove-background', {
    ...
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold flex items-center gap-2">
          <Wand2 size={24} className="text-primary" /> Estudio de IA
        </h1>
        <p className="text-sm text-gray-500 mt-1">
          Genera imágenes profesionales para tus productos, banners y redes sociales con FLUX.
          Cada generación cuesta entre $0.003 (rápida) y $0.03 (alta calidad) USD.
        </p>
      </div>
